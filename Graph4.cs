...
namespace ZedGraphSample
{
	public class Graph4
	{
		public List<Points4> CreateGraph4(ZedGraphControl zgc, AllParam ap1, List<Points3> inputlist34)
		{
			...
			dt4 = ap1.Dt4;
			r4 = Convert.ToDouble(ap1.Otr4.Replace(".", ",")); // коэф отражения
			t4 = 1 - r4; ;   //
			etalon4 = Convert.ToDouble(ap1.Etal4.Replace(".", ",")) * 1000000 + dt4;
			wave1 = Convert.ToDouble(ap1.Wave.Replace(".", ","));
			n4 = Convert.ToDouble(ap1.Prel4.Replace(".", ","));
			...
			double dlym = (wave1 * wave1) / (2 * etalon4 * n4); //расстояние между спектральными максимумами 

			PointPairList list4 = new PointPairList();
			List<Points4> list45 = new List<Points4>();
			int i = 0;
			...
			if (ap1.Indicator == 1)
			{
				...
				for (double x = wave1 - dwave1; x <= wave1 + dwave2; x += stap)
				{
					...
					double undcos = (2 * Math.PI / x) * 2 * etalon4 * n4;
					double y = inputlist34[i].y34 * (Math.Pow(t4, 2) / (1 + Math.Pow(r4, 4) - 2 * Math.Pow(r4, 2) * Math.Cos(undcos)));
					Points4 p45 = new Points4();
					p45.x45 = x;
					p45.y45 = y;
					list45.Add(p45);
					list4.Add(x, y);
			...
			LineItem myCurve = myPane.AddCurve("4 эталона", list4, Color.Blue,
									SymbolType.Circle);
